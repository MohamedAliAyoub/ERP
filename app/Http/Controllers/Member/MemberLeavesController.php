...
//        dd( date_diff($multi_date,$multi_date_end) , $multi_date , $multi_date_end , $diff  );
    ...
    public function store(StoreLeave $request)
    {
        if ($request->duration == 'multiple') {
            ...
            $dateInsert = Carbon::createFromFormat($this->global->date_format, $request->multi_date)->format('Y-m-d');
            $dateInsert_end = Carbon::createFromFormat($this->global->date_format, $request->multi_date_end)->format('Y-m-d');
            $multiDates = $multiDates->forget($removedIds);
                    $leave = new Leave();
                    ...
                    $leave->end_leave_date = $dateInsert_end;
                    $leave->days_count = $multiDates->count();
