...
class EmployeesDataTable extends BaseDataTable
{
    ...
    public function query(User $model)
    {
        ...
        if (isset(auth()->user()->manger->manger_id) == auth()->id()) {

            $admin = auth()->user()->manger;
            $employe = EmployeeDetails::where('department_id' , $admin->id)->get('user_id')->pluck('user_id');
            $users= $users->whereIn('users.id' , $employe);
        }
