...
@section('content')
                    ...
                    @forelse($pendingLeaves as $key=>$pendingLeave)
                        <div class="col-md-3 m-b-25">
                            ...
                            <div class="text-center bg-light p-t-20 p-b-20 m-l-n-25 m-r-n-25">
                                @if($pendingLeave->duration == "multiple")
    ...
    {{--Ajax Modal--}}
    <div class="modal fade bs-modal-md in" id="eventDetailModal1" role="dialog" aria-labelledby="myModalLabel"
         aria-hidden="true">
        <div class="modal-dialog modal-lg" id="modal-data-application">
            <div class="modal-content">
                <div class="modal-header">
...
<script>
    ...
    $('.leave-action').click(function () {
        ...
        var leaveId = $(this).data('leave-id');
        var searchQuery = "?leave_action="+action+"&leave_id="+leaveId;
        var url = '{!! route('admin.leaves.show-accept-modal') !!}'+searchQuery;
        $('#modelHeading').html('Reject Reason');
        $.ajaxModal('#eventDetailModal1', url);
    });
